...
export const getCountingBoundaryNames = async (payload) => {
  try {
    ...
    if (response.status === 200) {
      result.status = 1;
      result.body = {
        count_boundary_names: response.data.counting_boundary_names,
      };
    } else {
      result.status = 0;
      result.body = {};
...
//     boundary_name : string
// }
export async function getCountingBoundary(payload) {
  try {
    const result = {
      status: 0,
      body: {},
    ...
    if (response.status === 200) {
      ...
      result.body = {
        boundary_data: response.data.counting_boundary_data,
      };
    } else {
      result.status = 0;
      result.body = {};
